"""Configurations du modèle LTX-2.3.

Ce module définit les dataclasses de configuration pour la famille LTX-2.3.
Par rapport à la config LTX-2 d'origine, ce fichier est simplifié à LTX-2.3
uniquement : le flag `has_prompt_adaln` vaut True par défaut (LTX-2.3 utilise
toujours l'adaptive layer norm conditionné par le prompt) et `double_precision_rope`
...
class LTXModelType(Enum):
    """Modalité du modèle LTX. Le checkpoint distillé est AudioVideo (A/V conjoint)."""
...
class BaseModelConfig:
    ...
    def from_dict(cls, params: dict[str, Any]) -> "BaseModelConfig":
        """Crée une config à partir d'un dictionnaire en filtrant les paramètres valides."""
        return cls(
            **{
                k: v
    ...
    def to_dict(self) -> dict[str, Any]:
        """Exporte la config sous forme de dictionnaire."""
        result = {}
...
class LTXModelConfig(BaseModelConfig):
    """Configuration du transformer LTX-2.3.

    Les valeurs par défaut sont calibrées pour LTX-2.3-distilled. Le flag
    `has_prompt_adaln` vaut True pour LTX-2.3 (vs False pour l'ancien LTX-2 qui
    n'est pas pris en charge par ce paquet).
    """

    # Type de modèle
    model_type: LTXModelType = LTXModelType.AudioVideo

    # Config du transformer vidéo
    ...
    # Config des embeddings de position
    positional_embedding_theta: float = 10000.0
    positional_embedding_max_pos: Optional[List[int]] = None
    audio_positional_embedding_max_pos: Optional[List[int]] = None
    use_middle_indices_grid: bool = True
    rope_type: LTXRopeType = LTXRopeType.INTERLEAVED
    double_precision_rope: bool = True  # Valeur par défaut LTX-2.3

    # Config des timesteps
    ...
    # LTX-2.3 : adaptive layer norm conditionné par le prompt (True par défaut)
    # Contrôle : gate_logits dans l'attention, scale_shift_table à 9 paramètres,
    # prompt_adaln_single, prompt_scale_shift_table par bloc,
    # suppression de caption_projection
    ...
    def __post_init__(self):
        """Définit les valeurs par défaut après initialisation."""
        ...
        # Si un checkpoint se déclare non-2.3 (pas de prompt adaln), on retombe sur
        # un RoPE simple précision par sécurité. LTX-2.3 (le modèle pris en charge)
        # utilise la double précision.
        if not self.has_prompt_adaln:
            self.double_precision_rope = False

        # Conversion des valeurs d'enum en chaînes lors d'un chargement depuis dict
    ...
    def get_audio_config(self) -> Optional[TransformerConfig]:
        """Renvoie la configuration du transformer audio."""
...
class CausalityAxis(Enum):
    """Énumération de l'axe de causalité pour les convolutions causales."""
