...
from ...utils.common import to_denoised
...
class TransformerArgsPreprocessor:
    ...
    def _prepare_attention_mask(
        ...
        # Vérifie si déjà en float
        if attention_mask.dtype in [mx.float16, mx.float32, mx.bfloat16]:
            return attention_mask

        # Conversion d'un masque booléen/int en masque float
        # 0 -> -inf (masqué), 1 -> 0 (non masqué)
    ...
    def prepare(self, modality: Modality) -> TransformerArgs:
        ...
        # Utilisation des embeddings de position précalculés s'ils sont fournis (évite un recalcul coûteux du RoPE)
        ...
        # Timestep conditionné par le prompt (LTX-2.3) — utilise la valeur sigma brute, pas les timesteps par token
...
class MultiModalTransformerArgsPreprocessor:
    ...
    def prepare(self, modality: Modality) -> TransformerArgs:
        ...
        # Préparation des embeddings de position cross-modaux
        ...
        # Préparation des embeddings de timestep pour la cross-attention
...
class LTXModel(nn.Module):
    ...
    def __init__(self, config: LTXModelConfig):
        ...
        # Initialisation des composants cross-modaux
        if (
    ...
    def _process_transformer_blocks(
        ...
        """Traitement à travers tous les blocs transformer.

        Args:
            stg_video_blocks : indices des blocs où l'auto-attention vidéo est sautée (STG).
            stg_audio_blocks : indices des blocs où l'auto-attention audio est sautée (STG).
            skip_cross_modal : saute toute la cross-attention A2V/V2A (isolation des modalités).
        """
    ...
    def _process_output(
        ...
        # scale_shift_table : (2, dim) -> expand vers (1, 1, 2, dim)
        # embedded_timestep : (B, 1, dim) -> expand vers (B, 1, 1, dim)
        table_expanded = scale_shift_table[None, None, :, :]  # (1, 1, 2, dim)
        timestep_expanded = embedded_timestep[:, :, None, :]  # (B, 1, 1, dim)

        # Combinaison : (1, 1, 2, dim) + (B, 1, 1, dim) diffuse vers (B, 1, 2, dim)
        scale_shift_values = table_expanded + timestep_expanded

        # Extraction de shift et scale (le premier index est shift, le second scale)
    ...
    def __call__(
        ...
        Args:
            video : entrée de la modalité vidéo.
            audio : entrée de la modalité audio.
            stg_video_blocks : indices des blocs où l'auto-attention vidéo est sautée (STG).
            stg_audio_blocks : indices des blocs où l'auto-attention audio est sautée (STG).
            skip_cross_modal : saute toute la cross-attention A2V/V2A (isolation des modalités).
        """
        ...
        if not self.model_type.is_video_enabled() and video is not None:
            raise ValueError("La vidéo n'est pas activée pour ce modèle")
        if not self.model_type.is_audio_enabled() and audio is not None:
            raise ValueError("L'audio n'est pas activé pour ce modèle")
        ...
        # Traitement des blocs transformer
    ...
    def sanitize(self, weights: dict) -> dict:
        ...
        for key, value in weights.items():
            ...
            # Suppression du préfixe 'model.diffusion_model.'
